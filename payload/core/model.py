...
from .store import CLAIMED, CLOSED, DROPPED, WORKABLE, StateError
...
def frontier(graph: dict) -> list[dict]:
    """Prendibili con ogni blocker chiuso e nessuna domanda ancora aperta per una
    ...
    Un nodo sospeso sta qui come un aperto: il lavoro parziale congelato nel
    ticket non e' un motivo per non riprenderlo, e' il motivo per cui si riprende."""
    index = by_id(graph)
    return [n for n in graph["nodes"]
            if n["status"] in WORKABLE and all(is_done(blocker_of(index, n, d)) for d in n["blockedBy"])
...
def blocked(graph: dict) -> list[dict]:
    index = by_id(graph)
    return [n for n in graph["nodes"]
            if n["status"] in WORKABLE and not all(is_done(blocker_of(index, n, d)) for d in n["blockedBy"])]
