...
def _prompt(context: LaunchContext) -> str:
    ...
    return (
        ...
        f"'atlas release' on it, the node is yours. Read {ticket} including its Work section, "
        f"which may hold partial work from an earlier attempt, do the work, and record what you "
        f"do as you go with 'atlas log {node_id} \"<what you did>\"', one entry per activity: "
        f"close refuses a ticket with no entry. Then write the Answer section of the ticket "
        f"and close the node with "
        ...
        f"with a yes/no proposal instead of guessing or stalling silently: it puts the node on hold "
        f"without counting as a failure. If you must stop with work half done that is worth keeping, "
