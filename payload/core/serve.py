...
from . import render, serve_actions, serve_notify, serve_pairing
...
class Dashboard:
    """L'HTML in memoria, rigenerato quando l'mtime di graph.json avanza."""
    ...
    def aggiorna(self) -> bool:
        """Rigenera se serve: True se l'HTML e' stato rifatto. Un grafo
        momentaneamente illeggibile non fa rigenerare: la pagina a video resta
        quella di prima."""
        with self._lucchetto:
            mtime = self._mtime_grafo()
            if mtime is None:
                return False
            if self._html is not None and mtime <= self._mtime:
                return False
...
def _watch(server: Server) -> None:
    """La ronda: rigenera quando il grafo cambia e avverte i collegati."""
