...
STRINGS: dict[str, dict[str, str]] = {
    ...
    "parser.description": {"it": "Harness di task a grafo con runner Autopilot.",
                            "en": "Graph-based task harness with the Autopilot runner."},
    ...
    "help.give_up_motivo": {"it": "infeasible, missing-resource, blocked-environment, needs-redesign",
                            "en": "infeasible, missing-resource, blocked-environment, needs-redesign"},
    "help.give_up_dettaglio": {"it": "spiega il motivo a un umano, mai vuoto",
    ...
    "help.progress": {"it": "dichiara il passo raggiunto mentre lavori, con nota facoltativa",
                      "en": "declares the step you've reached while working, with an optional note"},
    "help.progress_step": {"it": "investigating, implementing, verifying, writing-answer, blocked",
    ...
    "help.run": {"it": "avvia un run Autopilot con parallelismo esplicito",
                 "en": "starts an Autopilot run with explicit parallelism"},
    ...
    "report.run_nessuno": {"it": "  nessun run Autopilot persistito",
                            "en": "  no persisted Autopilot run"},
    "report.run_titolo": {"it": "  run {id} · stato={status} · parallelism={parallelism}",
                           "en": "  run {id} · status={status} · parallelism={parallelism}"},
    "report.run_riga": {"it": "  run Autopilot: stato={status} · id={id}",
                         "en": "  Autopilot run: status={status} · id={id}"},
    "report.run_motivo": {"it": "    motivo: {reason}", "en": "    reason: {reason}"},
    "report.run_guasto": {"it": "failure={failure}", "en": "failure={failure}"},
    ...
    "progress.fatto": {"it": "  {id}: {step}", "en": "  {id}: {step}"},
    "progress.con_nota": {"it": " · {nota}", "en": " · {nota}"},
    "progress.fallito": {"it": "  segnale di avanzamento non scritto, il lavoro prosegue: {errore}",
                         "en": "  progress signal not written, work continues: {errore}"},
