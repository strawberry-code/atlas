...
Also ask for the graph's **slug** (kebab-case, e.g. `epic-auth`) and its **title**, if not already given. If the project already has graphs, show them with `python3 .atlas/atlas graphs`: maybe the work belongs to one of those.
...
```sh
python3 .atlas/atlas new <slug> -t "Graph title" -d "The destination, in one or two lines."
python3 .atlas/atlas new-script first-draft
```
...
```sh
python3 .atlas/atlas exec .atlas/scripts/001-first-draft.py
