...
A lock is orphaned when the process that took it no longer exists, or stalled when its heartbeat hasn't updated in too long. `atlas doctor` flags both cases, along with terminal nodes that don't flow into the final one and dashboards that are out of date: run it before declaring a graph done. With `lock.remote` active it also reports the remote lock's state, reachable, unreachable or declared but not injected, without ever dying.
