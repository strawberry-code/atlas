...
`atlas serve` tiene la dashboard viva su `http://127.0.0.1`, la rigenera quando `graph.json` cambia e spinge il reload al browser già aperto.
...
Un agente AFK dichiara quel che sta succedendo su un nodo attraverso cinque canali chiusi, mai un sesto inventato al volo: chi legge questi valori è un programma, non un modello, quindi fuori da questo elenco non c'è niente da interpretare.
...
- **Serve una persona**: `atlas ask-human <ID> -q "<proposta>"`. La proposta è un'alternativa binaria già formulata ("procedo con X, confermi?"), non una domanda aperta: il canale è a bottoni. Apre un'Interazione, mette il nodo in attesa senza contarlo come fallimento; alla risposta il nodo torna prendibile.
- **Lavoro parziale da congelare**: `atlas suspend <ID> -m "<dove sei arrivato e cosa manca>"`, quando devi fermarti con del lavoro fatto a metà che vale la pena tenere. Il nodo passa a `suspended`: molla il lucchetto, resta in frontiera e si riprende con `atlas take`, che stampa la nota. La nota è obbligatoria e finisce anche come voce nella Lavorazione del ticket, dove chi riprende legge il dettaglio; `suspend` rifiuta se quella sezione è ancora vuota, perché il lavoro parziale sta lì e senza non c'è niente da congelare. Non è una resa (il lavoro c'è) e non è un rilascio (il ticket non è vergine).
...
`close`, `give-up`, `ask-human` e `suspend` sono mutuamente esclusivi per costruzione: ciascuno pretende il nodo ancora rivendicato da chi chiama, quindi solo il primo che arriva vince e gli altri falliscono con lo stesso errore già esistente per "nodo non rivendicato".
...
Un lucchetto vivo con il turno fermo è lo stesso guasto vissuto da dentro, e `atlas doctor` non se ne accorge perché il processo che lo tiene è ancora in esecuzione. Se il lavoro su un nodo richiede una verifica lunga, come una suite di test lanciata in background, il turno non si chiude in attesa della notifica di completamento: o la verifica resta in primo piano finché non dà l'esito, oppure il turno prosegue con altro lavoro sul nodo e il log si rilegge quando serve davvero. Un turno fermo ad aspettare lascia il nodo rivendicato e il grafo bloccato per chiunque altro, con lo stesso effetto di un lucchetto orfano ma senza il suo sintomo.
...
Chi lavora un nodo scrive quel che fa nella sezione **Lavorazione** del ticket, anche il lavoro a metà, come registro a punti elenco: chi, quando, e a capo cosa ha fatto. Lo scrive `atlas log <ID> "cosa hai fatto"`, che firma la voce con l'identità e l'ora e la appende in coda; una voce per attività, chiamato spesso, non un riassunto a fine nodo. Il registro è ciò che sopravvive a un'interruzione: `suspend` pretende che ci sia già almeno una voce e ne aggiunge una con la nota, e `close` rifiuta un ticket che non ne ha nessuna, come rifiuta una Risposta vuota.
...
`close` verifica due cose, che la sezione **Lavorazione** del ticket abbia almeno una voce del registro e che la sezione **Risposta** sia compilata. Il resto lo dichiara chi chiude. Che la risposta sia anche vera non lo può verificare nessuna macchina, e l'unica difesa è che la scriva chi ha fatto il lavoro mentre ce l'ha fresco.
...
Un nodo spesso corrisponde a una risorsa fuori da Atlas: un'issue Jira, una PR GitHub, una pagina Confluence, un documento su OneDrive. `atlas link-external <ID> <url> [-l/--label "etichetta"]` la registra, aperto o già chiuso che sia il nodo: in dashboard diventa un pulsante nella scheda del nodo, che apre la risorsa in una scheda nuova, con l'etichetta data o il dominio dell'URL se non ne dai una. Un nodo senza link non mostra nessun pulsante. Solo `http`/`https`: `atlas validate` rifiuta qualunque altro schema.
