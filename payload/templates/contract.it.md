...
`close` accetta anche `-c/--costo` (un ordine di grandezza di quanto è costato, testo libero, niente di preciso) e `--artefatti` (i file prodotti, popolano il campo che il grafo già prevede). Senza `--artefatti`, in una repo git il campo si popola da solo con i file toccati da quando il nodo è stato rivendicato, esclusi quelli di `.atlas/`. È quel campo che permette a `doctor` di accorgersi di una scrittura dentro l'ambito di un nodo già chiuso; per lasciarlo vuoto di proposito basta `--artefatti` senza argomenti. Se un cancello rilascia un nodo invece di chiuderlo, `-r/--ragione` su `release` registra il perché come evento in mappa, non solo come un ritorno silenzioso alla frontiera.
