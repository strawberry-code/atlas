...
Automata (_avvia_tunnel_telegram/_ferma_tunnel_telegram), in isolamento dal
resto di execute()."""
...
class AvviaTunnelTelegram(unittest.TestCase):
    def test_nessun_thread_senza_configurazione_relay(self):
        with mock.patch.dict("os.environ", {}, clear=True):
            fermo, thread = automata._avvia_tunnel_telegram(_run())
    ...
    def test_thread_avviato_con_identita_di_sessione_e_fermato_pulito(self):
        env = {
        ...
        def fake_esegui(config, graph, run_id, on_event, stop):
            chiamate.append((graph, run_id))
            stop.wait()

        with mock.patch.dict("os.environ", env, clear=True), \
             mock.patch.object(automata.relay_client, "esegui", fake_esegui):
            fermo, thread = automata._avvia_tunnel_telegram(_run("mio-grafo", "run-42"))
            self.assertIsNotNone(thread)
            self.assertTrue(thread.is_alive())
            automata._ferma_tunnel_telegram(fermo, thread)

        self.assertFalse(thread.is_alive())
        self.assertEqual(chiamate, [("mio-grafo", "run-42")])

    def test_ferma_senza_thread_non_solleva(self):
        automata._ferma_tunnel_telegram(None, None)
